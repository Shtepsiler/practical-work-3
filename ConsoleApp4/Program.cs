...
namespace ConsoleApp3
{
    ...
    class Family
    {
        ...
        public static void GetOlder30Member()
        {

                      for(int i = 0; i < family.Count-1; i++)
                      {
           if (family[i].Name[0] > family[i+1].Name[0]  )
                          {
                    Person member = family[i];
                    family[i] = family[i+1];
                    family[i+1] = member;
            ...
            foreach (Person member in family)
            {
                if (member.Age > 30)
                {
                    Console.WriteLine($"{member.Name} is older then 30 ({member.Age})");
                }
